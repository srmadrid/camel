    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 i = 0; i < size; i++) {
                //cml_fraction_div(NULL, &scalar, &right->fracd[i], &out->fracd[i]);
            }
        } else  {
            //scalar = right->fracd[0];
            for (u32 i = 0; i < size; i++) {
                //cml_fraction_div(NULL, &left->fracd[i], &scalar, &out->fracd[i]);
            }
        }
    } else {
        for (u32 i = 0; i < size; i++) {
                //cml_fraction_div(NULL, &left->fracd[i], &right->fracd[i], &out->fracd[i]);
        }
    }
