/** @file cml_matrixI8_print.c
 * 
 * @brief Print function for matrices holding i8's.
 *
...
CML_Status cml_matrixI8_print(CML_Allocator *allocator, const CML_Matrix *matrix) {
    if (matrix == NULL) {
    ...
    for (u32 r = 0; r < matrix->rows; r++) {
        ...
        for (u32 c = 0; c < matrix->columns; c++) {
            i8 number = matrix->i8d[r*matrix->columns + c];
            u32 length;
