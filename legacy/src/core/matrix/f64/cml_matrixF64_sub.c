/** @file cml_matrixF64_sub.c
 * 
 * @brief Subtract function for matrices holding f64's.
 *
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 i = 0; i < size; i++) {
                out->f64d[i] = scalar - right->f64d[i];
            }
        } else  {
            scalar = right->f64d[0];
            for (u32 i = 0; i < size; i++) {
                out->f64d[i] = left->f64d[i] - scalar;
            }
        }
    } else {
        for (u32 i = 0; i < size; i++) {
            out->f64d[i] = left->f64d[i] - right->f64d[i];
        }
    }
