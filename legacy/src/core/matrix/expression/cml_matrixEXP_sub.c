    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 i = 0; i < size; i++) {
                //cml_expression_sub(NULL, &scalar, &right->expd[i], &out->expd[i]);
            }
        } else  {
            //scalar = right->expd[0];
            for (u32 i = 0; i < size; i++) {
                //cml_expression_sub(NULL, &left->expd[i], &scalar, &out->expd[i]);
            }
        }
    } else {
        for (u32 i = 0; i < size; i++) {
            //cml_expression_sub(NULL, &left->expd[i], &right->expd[i], &out->expd[i]);
        }
    }
