/** @file cml_matrixI8_mult.c
 * 
 * @brief Multiplication function for matrices holding i8's.
 *
...
CML_Status cml_matrixI8_mult(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            ...
            for (u32 c = 0; c < out->columns; c++) {
                    out->i8d[r*out->columns + c] += left->i8d[r*left->columns + k]*right->i8d[k*right->columns + c];
                }
            }
        }
    }
