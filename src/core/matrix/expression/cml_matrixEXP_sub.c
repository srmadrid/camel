/** @file cml_matrixEXP_sub.c
 * 
 * @brief Subtract function for matrices holding Expression's.
 *
...
CML_Status cml_matrixEXP_sub(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_expression_sub(NULL, &scalar, &right->expd[r*out->columns + c], &out->expd[r*out->columns + c]);
                }
            }
        } else  {
            //scalar = right->expd[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_expression_sub(NULL, &left->expd[r*out->columns + c], &scalar, &out->expd[r*out->columns + c]);
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_expression_sub(NULL, &left->expd[r*out->columns + c], &right->expd[r*out->columns + c], &out->expd[r*out->columns + c]);
            }
        }
    }
