/** @file cml_matrixEXP_mult.c
 * 
 * @brief Multiplication function for matrices holding Expression's.
 *
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_expression_mult(NULL, &scalar, &right->expd[r*out->columns + c], &out->expd[r*out->columns + c]);
                }
            }
        } else  {
            scalar = right->expd[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_expression_mult(NULL, &left->expd[r*out->columns + c], &scalar, &out->expd[r*out->columns + c]);
                }
            }
        }
    } else {
        CML_Expression tmp;
        //CML_Status result = cml_expression_init(out->allocator, 2, &tmp);
        //if (result != CML_SUCCESS) {
        //    return result;
        //}
        for (u32 r = 0; r < out->rows; r++) {
            ...
            for (u32 c = 0; c < out->columns; c++) {
                    //cml_expression_mult(NULL, &left->expd[r*left->columns + k], &right->expd[k*right->columns + c], &tmp);
                    //cml_expression_add_inplace(&tmp, &out->expd[r*out->columns + c]);
                }
            }
        }
        //cml_expression_destroy(&tmp);
    }
