/** @file cml_matrixI16_divew.c
 * 
 * @brief Element-wise division function for matrices holding i16's.
 *
...
CML_Status cml_matrixI16_divew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->i16d[r*out->columns + c] = scalar/right->i16d[r*out->columns + c];
                }
            }
        } else  {
            scalar = right->i16d[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->i16d[r*out->columns + c] = left->i16d[r*out->columns + c]/scalar;
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->i16d[r*out->columns + c] = left->i16d[r*out->columns + c]/right->i16d[r*out->columns + c];
            }
        }
    }
