/** @file cml_matrixI16_mult.c
 * 
 * @brief Multiplication function for matrices holding i16's.
 *
...
CML_Status cml_matrixI16_mult(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            ...
            for (u32 c = 0; c < out->columns; c++) {
                    out->i16d[r*out->columns + c] += left->i16d[r*left->columns + k]*right->i16d[k*right->columns + c];
                }
            }
        }
    }
