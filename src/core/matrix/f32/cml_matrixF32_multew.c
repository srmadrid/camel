/** @file cml_matrixF32_multew.c
 * 
 * @brief Element-wise multiplication function for matrices holding f32's.
 *
...
CML_Status cml_matrixF32_multew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->f32d[r*out->columns + c] = scalar*right->f32d[r*out->columns + c];
                }
            }
        } else  {
            scalar = right->f32d[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->f32d[r*out->columns + c] = left->f32d[r*out->columns + c]*scalar;
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->f32d[r*out->columns + c] = left->f32d[r*out->columns + c]*right->f32d[r*out->columns + c];
            }
        }
    }
