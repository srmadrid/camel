/** @file cml_matrixU64_transpose.c
 * 
 * @brief Transpose function for matrices holding u64's.
 *
...
CML_Status cml_matrixU64_transpose(CML_Allocator *allocator, const CML_Matrix *A, CML_Matrix *out) {
    ...
    if (allocator == NULL) {
        ...
        CML_Status result = cml_matrix_init(allocator, A->columns, A->rows, CML_U64, out);
    ...
    for (u32 r = 0; r < out->rows; r++) {
        for (u32 c = 0; c < out->columns; c++) {
            out->u64d[r*out->columns + c] = A->u64d[c*out->rows + r];
        }
    }
