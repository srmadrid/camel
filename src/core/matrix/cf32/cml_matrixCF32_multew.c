/** @file cml_matrixCF32_multew.c
 * 
 * @brief Element-wise multiplication function for matrices holding cf32's.
 *
...
CML_Status cml_matrixCF32_multew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->cf32d[r*out->columns + c] = left->cf32d[r*out->columns + c]*right->cf32d[r*out->columns + c];
            }
        }
    }
