/** @file cml_matrixBINT_add.c
 * 
 * @brief Add function for matrices holding Bigint's.
 *
...
CML_Status cml_matrixBINT_add(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_bigint_add(NULL, &scalar, &right->bigintd[r*out->columns + c], &out->bigintd[r*out->columns + c]);
                }
            }
        } else  {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_bigint_add(NULL, &left->bigintd[r*out->columns + c], &scalar, &out->bigintd[r*out->columns + c]);
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_bigint_add(NULL, &left->bigintd[r*out->columns + c], &right->bigintd[r*out->columns + c], &out->bigintd[r*out->columns + c]);
            }
        }
    }
