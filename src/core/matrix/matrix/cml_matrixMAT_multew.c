/** @file cml_matrixMAT_multew.c
 * 
 * @brief Element-wise multiplication function for matrices holding Matrix's.
 *
...
CML_Status cml_matrixMAT_multew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    cml_matrix_multew(NULL, &scalar, &right->matd[r*out->columns + c], &out->matd[r*out->columns + c]);
                }
            }
        } else  {
            scalar = right->matd[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    cml_matrix_multew(NULL, &left->matd[r*out->columns + c], &scalar, &out->matd[r*out->columns + c]);
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                cml_matrix_multew(NULL, &left->matd[r*out->columns + c], &right->matd[r*out->columns + c], &out->matd[r*out->columns + c]);
            }
        }
    }
