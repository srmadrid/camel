/** @file cml_matrixMAT_add_inplace.c
 * 
 * @brief In-place add function for matrices holding Matrix's.
 *
    ...
    if (rightIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                cml_matrix_add_inplace(&scalar, &out->matd[r*out->columns + c]);
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                cml_matrix_add_inplace(&right->matd[r*out->columns + c], &out->matd[r*out->columns + c]);
            }
        }
    }
