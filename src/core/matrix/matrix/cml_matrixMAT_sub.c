    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 i = 0; i < size; i++) {
                cml_matrix_sub(NULL, &scalar, &right->matd[i], &out->matd[i]);
            }
        } else  {
            scalar = right->matd[0];
            for (u32 i = 0; i < size; i++) {
                cml_matrix_sub(NULL, &left->matd[i], &scalar, &out->matd[i]);
            }
        }
    } else {
        for (u32 i = 0; i < size; i++) {
            cml_matrix_sub(NULL, &left->matd[i], &right->matd[i], &out->matd[i]);
        }
    }
