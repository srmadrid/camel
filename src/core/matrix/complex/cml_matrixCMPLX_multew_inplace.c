/** @file cml_matrixCMPLX_multew_inplace.c
 * 
    ...
    if (rightIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_complex_mult_inplace(&scalar, &out->complexd[r*out->columns + c]);
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_complex_mult_inplace(&right->complexd[r*out->columns + c], &out->complexd[r*out->columns + c]);
            }
        }
    }
