/** @file cml_matrixCMPLX_sub.c
 * 
 * @brief Subtract function for matrices holding Complex's.
 *
...
CML_Status cml_matrixCMPLX_sub(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_complex_sub(NULL, &scalar, &right->complexd[r*out->columns + c], &out->complexd[r*out->columns + c]);
                }
            }
        } else  {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_complex_sub(NULL, &left->complexd[r*out->columns + c], &scalar, &out->complexd[r*out->columns + c]);
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_complex_sub(NULL, &left->complexd[r*out->columns + c], &right->complexd[r*out->columns + c], &out->complexd[r*out->columns + c]);
            }
        }
    }
