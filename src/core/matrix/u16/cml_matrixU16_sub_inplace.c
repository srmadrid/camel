/** @file cml_matrixU16_sub_inplace.c
 * 
 * @brief In-place subtract function for matrices holding u16's.
 *
    ...
    if (rightIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->u16d[r*out->columns + c] -= scalar;
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->u16d[r*out->columns + c] -= right->u16d[r*out->columns + c];
            }
        }
    }
