/** @file cml_matrixU16_multew.c
 * 
 * @brief Element-wise multiplication function for matrices holding u16's.
 *
...
CML_Status cml_matrixU16_multew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->u16d[r*out->columns + c] = scalar*right->u16d[r*out->columns + c];
                }
            }
        } else  {
            scalar = right->u16d[0];
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->u16d[r*out->columns + c] = left->u16d[r*out->columns + c]*scalar;
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->u16d[r*out->columns + c] = left->u16d[r*out->columns + c]*right->u16d[r*out->columns + c];
            }
        }
    }
