/** @file cml_matrixU16_mult.c
 * 
 * @brief Multiplication function for matrices holding u16's.
 *
...
CML_Status cml_matrixU16_mult(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            ...
            for (u32 c = 0; c < out->columns; c++) {
                    out->u16d[r*out->columns + c] += left->u16d[r*left->columns + k]*right->u16d[k*right->columns + c];
                }
            }
        }
    }
