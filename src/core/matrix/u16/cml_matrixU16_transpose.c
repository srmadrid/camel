/** @file cml_matrixU16_transpose.c
 * 
 * @brief Transpose function for matrices holding u16's.
 *
...
CML_Status cml_matrixU16_transpose(CML_Allocator *allocator, const CML_Matrix *A, CML_Matrix *out) {
    ...
    if (allocator == NULL) {
        ...
        CML_Status result = cml_matrix_init(allocator, A->columns, A->rows, CML_U16, out);
    ...
    for (u32 r = 0; r < out->rows; r++) {
        for (u32 c = 0; c < out->columns; c++) {
            out->u16d[r*out->columns + c] = A->u16d[c*out->rows + r];
        }
    }
