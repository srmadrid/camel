/** @file cml_matrixFRAC_divew.c
 * 
 * @brief Element-wise division function for matrices holding Fraction's.
 *
...
CML_Status cml_matrixFRAC_sub(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_fraction_div(NULL, &scalar, &right->fracd[r*out->columns + c], &out->fracd[r*out->columns + c]);
                }
            }
        } else  {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    //cml_fraction_div(NULL, &left->fracd[r*out->columns + c], &scalar, &out->fracd[r*out->columns + c]);
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                //cml_fraction_div(NULL, &left->fracd[r*out->columns + c], &right->fracd[r*out->columns + c], &out->fracd[r*out->columns + c]);
            }
        }
    }
