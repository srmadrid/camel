/** @file cml_matrixCF64_multew.c
 * 
 * @brief Element-wise multiplication function for matrices holding cf64's.
 *
...
CML_Status cml_matrixCF64_multew(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        if (leftIsScalar) {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->cf64d[r*out->columns + c] = scalar*right->cf64d[r*out->columns + c];
                }
            }
        } else  {
            ...
            for (u32 r = 0; r < out->rows; r++) {
                for (u32 c = 0; c < out->columns; c++) {
                    out->cf64d[r*out->columns + c] = left->cf64d[r*out->columns + c]*scalar;
                }
            }
        }
    } else {
        for (u32 r = 0; r < out->rows; r++) {
            for (u32 c = 0; c < out->columns; c++) {
                out->cf64d[r*out->columns + c] = left->cf64d[r*out->columns + c]*right->cf64d[r*out->columns + c];
            }
        }
    }
