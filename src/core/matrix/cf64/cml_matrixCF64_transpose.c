/** @file cml_matrixCF64_transpose.c
 * 
 * @brief Transpose function for matrices holding cf64's.
 *
...
CML_Status cml_matrixCF64_transpose(CML_Allocator *allocator, const CML_Matrix *A, CML_Matrix *out) {
    ...
    if (allocator == NULL) {
        ...
        CML_Status result = cml_matrix_init(allocator, A->columns, A->rows, CML_CF64, out);
    ...
    for (u32 r = 0; r < out->rows; r++) {
        for (u32 c = 0; c < out->columns; c++) {
            out->cf64d[r*out->columns + c] = A->cf64d[c*out->rows + r];
        }
    }
