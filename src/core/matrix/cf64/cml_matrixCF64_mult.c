/** @file cml_matrixCF64_mult.c
 * 
 * @brief Multiplication function for matrices holding cf64's.
 *
...
CML_Status cml_matrixCF64_mult(CML_Allocator *allocator, const CML_Matrix *left, const CML_Matrix *right, CML_Matrix *out) {
    ...
    if (oneIsScalar) {
        ...
        for (u32 r = 0; r < out->rows; r++) {
            ...
            for (u32 c = 0; c < out->columns; c++) {
                    out->cf64d[r*out->columns + c] += left->cf64d[r*left->columns + k]*right->cf64d[k*right->columns + c];
                }
            }
        }
    }
